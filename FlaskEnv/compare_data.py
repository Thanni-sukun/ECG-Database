...
df_telemetry = pd.read_csv("telemetry_data.csv")
df_all_lead = pd.read_excel("1_sample_all_lead.xlsx")

# Limit to the first 1000 data points
df_telemetry = df_telemetry.head(1000)
df_all_lead = df_all_lead.head(1000)
...
# The ALL_LEAD.xlsx data is already in millivolts, but column names are different (CH1 to CH8)
...
fig.suptitle("ECG Data Comparison: telemetry_data.csv vs ALL_LEAD.xlsx (First 1000 Points)", fontsize=16)

# Plot each channel
for i, (telemetry_col, all_lead_col) in enumerate(zip(channels, all_lead_channels)):
    # Plot telemetry data in blue
    axes[i].plot(df_telemetry.index, df_telemetry[telemetry_col], label=f"{telemetry_col} (telemetry)", color="blue")
    # Plot ALL_LEAD data in red
    axes[i].plot(df_all_lead.index, df_all_lead[all_lead_col], label=f"CH{i+1} (ALL_LEAD)", color="red", alpha=0.7)
    # Add MSE to the plot
...
plt.savefig("ecg_comparison_plot.png")
plt.show()
